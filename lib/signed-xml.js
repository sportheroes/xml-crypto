...
SignedXml.prototype.getKeyInfo = function(prefix) {
  ...
  let oneliner = this.authnCertificate;
  oneliner = oneliner.replace(/-+BEGIN CERTIFICATE-+\r?\n?/, '');
  oneliner = oneliner.replace(/-+END CERTIFICATE-+\r?\n?/, '');
  oneliner = oneliner.replace(/\r?\n|\r/g, '');

  res += "<" + currentPrefix + "KeyInfo>";
  res += "<" + currentPrefix + "X509Data>";
  res += "<" + currentPrefix + "X509Certificate>" + oneliner + "</" + currentPrefix + "X509Certificate>";
  res += "</" + currentPrefix + "X509Data>";
  res += "</" + currentPrefix + "KeyInfo>";
